...
export function themeEditorPage(themeValue: string | null) {
  const theme = safeTheme(themeValue);
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Visual Editor · ${theme}</title><style>${editorStyles}</style></head><body><header class="editor-bar"><a href="/admin/themes">← Giao diện</a><b>${theme}</b><label>Trang<select id="scope"><option value="landing">Landing</option><option value="auth">Auth</option><option value="customer">Customer</option></select></label><div class="devices"><button data-device="desktop">Desktop</button><button data-device="tablet">Tablet</button><button data-device="mobile">Mobile</button></div><span id="state">Chưa áp dụng</span><button id="undo">Hoàn tác</button><button id="reset">Khôi phục mặc định</button><button id="save">Lưu bản nháp</button><button id="apply">Áp dụng</button><a id="newtab" target="_blank" rel="noopener">Mở bản xem trước</a></header><main class="editor"><aside id="controls">${controls}</aside><section class="canvas"><iframe id="preview" title="Bản xem trước website thực"></iframe></section></main><script>${editorScript(theme)}</script></body></html>`;
}
